...
Workflow Steps:
1) Build a popularity baseline from train interactions.
2) Build MF latent factors with truncated SVD.
3) Train two-tower embeddings with contrastive learning.
4) Apply configured early-stop rule on validation Recall@K or training loss.
5) Return numpy artifacts used by train/eval/serve stages.
...
def _train_two_tower(
    ...
    """What: Train two-tower retrieval embeddings with contrastive objective.
    Why: Learns user/item representations for ANN-based personalized retrieval.

    Beginner view:
    - Inputs are user->purchased-item interactions.
    ...
    # ===== Step 5: Build Validation Cache (For Configured Early Stop) =====
    # Precompute validation user/item indices once to keep epoch loop simpler.
    ...
    def _validation_recall_at_k() -> float:
        ...
        Why: Stops training when ranking quality stops improving.
        """
    ...
    # For each batch:
    # - Build user vectors and their positive item vectors.
    ...
    for epoch in range(1, epochs + 1):
        ...
        for start in range(0, len(positive_array), max(1, batch_size)):
            ...
            # Row i should match column i (its own purchased item).
            # Other columns in the batch act as negatives.
            similarity_logits = (user_embeddings_batch @ positive_item_embeddings_batch.T) / float(temperature)  # [B, B]
            if negative_samples > 0:
                ...
                similarity_logits = torch.cat([similarity_logits, sampled_negative_logits], dim=1)  # [B, B+N]
