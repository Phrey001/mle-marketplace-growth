...
user_rollups AS (
  SELECT
    ...
    count(DISTINCT CASE WHEN positive_events.event_date >= params.as_of_date - INTERVAL 59 DAY THEN positive_events.invoice_id END) AS frequency_60d,
    ...
    count(DISTINCT CASE WHEN positive_events.event_date >= params.as_of_date - INTERVAL 119 DAY THEN positive_events.invoice_id END) AS frequency_120d,
    ...
    sum(CASE WHEN positive_events.event_date >= params.as_of_date - INTERVAL 59 DAY THEN positive_events.line_revenue ELSE 0 END) AS monetary_60d,
    ...
    sum(CASE WHEN positive_events.event_date >= params.as_of_date - INTERVAL 119 DAY THEN positive_events.line_revenue ELSE 0 END) AS monetary_120d
  FROM positive_events, params
  GROUP BY positive_events.user_id
),
invoice_totals_60d AS (
  SELECT
    positive_events.user_id,
    positive_events.invoice_id,
    sum(positive_events.line_revenue) AS invoice_revenue
  FROM positive_events, params
  WHERE positive_events.event_date >= params.as_of_date - INTERVAL 59 DAY
...
basket_60d AS (
  SELECT
    user_id,
    avg(invoice_revenue) AS avg_basket_value_60d
  FROM invoice_totals_60d
  GROUP BY user_id
),
basket_90d AS (
  SELECT
...
invoice_totals_120d AS (
  SELECT
    positive_events.user_id,
    positive_events.invoice_id,
    sum(positive_events.line_revenue) AS invoice_revenue
  FROM positive_events, params
  WHERE positive_events.event_date >= params.as_of_date - INTERVAL 119 DAY
  GROUP BY positive_events.user_id, positive_events.invoice_id
),
basket_120d AS (
  SELECT
    user_id,
    avg(invoice_revenue) AS avg_basket_value_120d
  FROM invoice_totals_120d
  GROUP BY user_id
)
SELECT
  users.user_id,
  ...
  round(coalesce(basket_60d.avg_basket_value_60d, 0), 4) AS avg_basket_value_60d,
  round(coalesce(basket_90d.avg_basket_value_90d, 0), 4) AS avg_basket_value_90d,
  round(coalesce(basket_120d.avg_basket_value_120d, 0), 4) AS avg_basket_value_120d,
...
LEFT JOIN basket_60d ON users.user_id = basket_60d.user_id
LEFT JOIN basket_90d ON users.user_id = basket_90d.user_id
LEFT JOIN basket_120d ON users.user_id = basket_120d.user_id
